...
$(function () {
  ...
  var cleanInput = __webpack_require__(92);
  ...
  var searchBtn = $('.search_text');
  var input = inputBox.find('input');
  var resultDom = $('.search_result_box');
  var resultCloseBtn = resultDom.find('.close_btn');
  var resultUl = $('.result_ul');
  var searchVal = getCookie("search");
  var cookieValFun = function cookieValFun() {
    var val = input.val();
    if ($.trim(val) != '') {
      searchVal = searchVal + ',' + val;
      var cookieArr = searchVal.split(',');
      var newArr = cookieArr.unique1();
  ...
  if (searchVal) {
    ...
    var inputVal = getCookie("search");
    var valArr = inputVal.split(',');
    valArr.shift();
    if (valArr.length < 10) {
      for (var i = 0; i < valArr.length; i++) {
        var creatLi = $('<li class="result_li">' + valArr[i] + '</li>');
        resultUl.append(creatLi);
      }
    } else {
      for (var _i = 0; _i < 10; _i++) {
        var _creatLi = $('<li class="result_li">' + valArr[_i] + '</li>');
        resultUl.append(_creatLi);
      }
    }
    var resultLi = resultUl.find('.result_li');
    resultLi.each(function (index) {
      var $this = resultLi.eq(index);
...
var inputClean = function () {
    ...
    var inputFun = function () {
        function inputFun(dom) {
            (0, _classCallCheck3.default)(this, inputFun);
        ...
        (0, _createClass3.default)(inputFun, [{
            key: 'init',
            value: function init() {
                var _this = this;
                ...
                this.input.on('keyup', function () {
                    _this.inputVal = _this.input.val();
                    _this.empty();
                });
                this.bind();
            }
            ...
            value: function bind() {
                ...
                this.cleanBtn.on('click', function () {
                    _this2.input.val('');
                });
            }
        }]);
